  ...
  <div class="prod-page">
    ...
    <div class="sp-table-wrap">
      <div class="sp-table-scroll">
        <div class="sp-table-filters" style="padding:0.9rem 1.1rem; border-bottom:1px solid var(--sp-border); background:transparent">
          <div style="display:flex; gap:1rem; align-items:center; flex-wrap:wrap">
            <div style="display:flex; gap:.5rem; align-items:center; flex:0 1 350px">
              <label style="display:flex; gap:.5rem; align-items:center; width:100%">
                Producto:
                <div style="position:relative; width:100%">
                  <input v-model="filterProductSearch" type="text" class="sp-drawer-input" placeholder="Buscar..." />
                  <div v-if="filterProductSearch && filterFilteredProducts.length" class="product-list" style="position:absolute; top:100%; left:0; right:0; margin-top:0.25rem; background:white; border:1px solid var(--sp-border); border-radius:0.375rem; z-index:10">
                    <div style="display:flex;flex-direction:column;gap:0">
                      <button v-for="p in filterFilteredProducts" :key="p.id" type="button" class="sp-drawer-btn sp-drawer-btn--ghost" style="text-align:left; border-radius:0; border-bottom:1px solid var(--sp-border); padding:0.5rem 0.75rem" @click="selectFilterProduct(p)">
                        {{ p.name }}
                      </button>
                    </div>
                  </div>
              ...
              <div v-if="filterSelectedProduct" style="padding:0.25rem 0.5rem; background:#f0f0f0; border-radius:0.25rem; font-size:0.875rem; white-space:nowrap">
                {{ filterSelectedProduct.name }}
                <button type="button" @click="filterSelectedProduct = null; filters.productId = null" style="margin-left:0.5rem; cursor:pointer; font-weight:bold">&times;</button>
              </div>
            </div>
            <label style="display:flex; gap:.5rem; align-items:center">Tipo:
            ...
            <button class="sp-page-btn" style="padding:.4rem .8rem; height:auto;" @click="applyFilters">Filtrar</button>
          </div>
        </div>
        ...
        <table class="sp-table">
          ...
          <tbody>
            <tr v-for="m in items" :key="m.id" class="sp-tr">
              ...
              <td class="sp-td">
                <span :style="{
                  color: (m.delta ?? 0) > 0 ? '#16a34a' : ((m.delta ?? 0) < 0 ? '#ef4444' : '#6b7280'),
                  fontWeight: 600
                }">{{ formatSigned(m.delta) }}</span>
              </td>
              <td class="sp-td">{{ m.reason }}</td>
              <td class="sp-td">{{ m.createdByName || m.createdBy }}</td>
              <td class="sp-td sp-td--muted">{{ new Date(m.createdAt).toLocaleString() }}</td>
            </tr>
      ...
      <div class="sp-table-footer" style="display:flex; align-items:center; justify-content:space-between; padding: .75rem 1rem; gap:1rem">
        ...
        <div style="display:flex; gap:0.35rem; align-items:center">
          <button v-for="p in totalPages" :key="p" @click="page = p - 1; fetchMovements()" :disabled="p - 1 === page" class="sp-table-btn" style="min-width:36px; padding:.45rem .6rem; font-size:0.875rem" :style="{ fontWeight: p - 1 === page ? 600 : 400, background: p - 1 === page ? '#3b82f6' : 'transparent', color: p - 1 === page ? 'white' : 'inherit' }">{{ p }}</button>
        </div>
      </div>
    </div>
          ...
          <div class="sp-drawer-body">
            <form @submit.prevent="submitMovement">
              <div class="sp-drawer-field">
                ...
                <div class="product-list" v-if="filteredProducts.length" style="margin-top:.5rem">
                  <div style="display:flex;flex-direction:column;gap:.35rem">
                    <button v-for="p in filteredProducts" :key="p.id" type="button" class="sp-drawer-btn sp-drawer-btn--ghost" @click="selectProduct(p)">
                      {{ p.name }} — stock: {{ p.stock ?? 0 }}
                    </button>
                  </div>
                </div>
                <div v-if="selectedProduct" style="margin-top:.5rem">Seleccionado: <strong>{{ selectedProduct.name }}</strong> (ID: {{ selectedProduct.id }})</div>
              </div>
              ...
              <div class="sp-drawer-footer">
                <button type="button" class="sp-drawer-btn sp-drawer-btn--ghost" @click="closeDrawer">Cancelar</button>
                <button type="submit" class="sp-drawer-btn sp-drawer-btn--primary">Crear</button>
              </div>
            </form>
          </div>
        </aside>
...
const filters = reactive<{ productId: number | null; type: string }>({ productId: null, type: '' })
...
// form state
const form = reactive<{ productId: number | null; movementType: string; quantity: number; reason: string }>({ productId: null, movementType: 'entry', quantity: 1, reason: '' })
...
async function fetchMovements() {
  ...
  error.value = null
  try {
    const params: any = { limit: limit.value, offset: page.value * limit.value }
    if (filters.productId) params.productId = filters.productId
    if (filters.type) params.type = filters.type
    const res: any = await $fetch('/api/admin/inventory/movements', { query: params })
...
function applyFilters() { page.value = 0; fetchMovements() }
...
function openDrawer() { formError.value = ''; showForm.value = true }
function closeDrawer() { showForm.value = false }

// backfill
async function runBackfill() {
  const toast = useAppToast()
  try {
    const res = await $fetch('/api/admin/inventory/backfill', { method: 'POST', body: {} })
    const created = res?.created ?? 0
    const processed = res?.processedOrders ?? 0
    toast.add({ title: 'Reconstrucción completa', description: `${created} movimientos creados para ${processed} pedidos`, color: 'success' })
    refreshKey.value++
  } catch (err) {
    const e:any = err
    toast.add({ title: 'Error', description: e?.message ?? String(e), color: 'error' })
  }
}
...
const filteredProducts = computed<any[]>(() => {
  ...
  return products.value.filter((p: any) => (p.name || '').toLowerCase().includes(q)).slice(0, 50)
})
...
const filterFilteredProducts = computed<any[]>(() => {
  ...
  return filterProducts.value.filter((p: any) => (p.name || '').toLowerCase().includes(q)).slice(0, 50)
})
...
async function submitMovement() {
  ...
  try {
    ...
    await $fetch('/api/admin/inventory/movements', { method: 'POST', body: form })
