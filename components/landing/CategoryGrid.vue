<template>
  <section v-if="shouldRender" :id="`seccion-${sectionId}`" class="cat-grid-section"
    ...
    <div v-if="loading" class="cat-grid cat-grid--4col">
      <div v-for="n in skeletonCount" :key="`sk-${n}`" class="cat-card cat-card--skeleton" :style="`--i: ${n - 1}`">
    ...
    <!-- Grid real -->
    <div v-else class="cat-grid" :class="[`cat-grid--${Math.min(categories.length, 4)}col`]">
      <a v-for="(cat, i) in categories" :key="cat.id ?? i"
        :href="cat.href ?? `/productos?categoria=${cat.slug}&page=1&limit=24`" class="cat-card" :style="`--i: ${i}`"
...
watch(() => props.sectionId, () => void loadCategories());

const route = useRoute();
watch(() => route.path, (p) => { if (p === "/") void loadCategories(); });
